...
class DetailViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    if let todo = todo {
      ...
      todoDescription.text = todo.descriptional
    } else {
      title = "New Todo"
      childButton.isSelected = true
    }*/
  ...
  @IBAction func tapDone(_ sender: AnyObject) {
    ...
    if let todo = todo {
      ...
      let uuid = UUID().uuidString
        todo = ToDoItem(id: uuid, image: image, title: todoTitleLabel.text!, date: todoDatePicker.date, description: todoDescription.text!)
        todos.append(todo!)
    }
